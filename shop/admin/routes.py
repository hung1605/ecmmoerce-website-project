...
def register():
    ...
    username_in_used = User.query.filter_by(username=form.username.data).first() 
    email_in_used = User.query.filter_by(email=form.email.data).first() 

    if username_in_used or email_in_used:
        if username_in_used:
